...
set "BUILDTOOLS_VARSALLPATH_2017=\Microsoft Visual Studio\2017\BuildTools\VC\Auxiliary\Build\vcvarsall.bat"
set "BUILDTOOLS_VARSALLPATH_2019=\Microsoft Visual Studio\2019\BuildTools\VC\Auxiliary\Build\vcvarsall.bat"
...
if exist "%ProgramFiles(X86)%%BUILDTOOLS_VARSALLPATH_2019%" (
"%ProgramFiles(X86)%%BUILDTOOLS_VARSALLPATH_2019%" %ARCHITECTURE%
) else (
    if exist "%ProgramFiles%%BUILDTOOLS_VARSALLPATH_2019%" (
        "%ProgramFiles%%BUILDTOOLS_VARSALLPATH_2019%" %ARCHITECTURE%
    ) else (
        if exist "%ProgramFiles%%BUILDTOOLS_VARSALLPATH_2017%" (
            "%ProgramFiles%%BUILDTOOLS_VARSALLPATH_2017%" %ARCHITECTURE%
        ) else (
            if exist "%ProgramFiles(X86)%%BUILDTOOLS_VARSALLPATH_2017%" (
                "%ProgramFiles(X86)%%BUILDTOOLS_VARSALLPATH_2017%" %ARCHITECTURE%
            ) else (
